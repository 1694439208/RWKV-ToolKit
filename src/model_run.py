...
############################################################################################################
class RWKV_RNN_ONNX(torch.nn.Module):
    def __init__(self, MODEL_NAME, RUN_DEVICE, model_type, n_layer, n_embd, ctx_len):
        ...
        t1=w["intput.parameter"].cpu().numpy().astype('int')
        self.n_layer = t1[1]
        self.n_embd = t1[2]
        self.ctx_len = t1[0]
        print(t1,self.n_layer,self.n_embd,self.ctx_len)     
        del w["intput.parameter"]  
    ...
    def sample_logits(self, out, temperature=1.0, top_p=0.7):
        ...
        #print(torch.tensor([1,probs]).shape)
        #return torch.multinomial(probs, num_samples=1)
        #dist = torch.distributions.Categorical(probs)
        '''weights = torch.Tensor([
            probs.tolist()
        ])'''
        return probs#torch.multinomial(weights, num_samples=1)[0]
    def forward(self, ctx, xx_att, aa_att, bb_att, xx_ffn):
        ...
        xx_ffn_r = torch.stack(xx_ffn_cd)#self.sample_logits(x.tolist())
