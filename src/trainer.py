    ...
    def train(self):
        ...
        def run_epoch(split):
            ...
            for it, (x, y) in pbar:
                ...
                if is_train:  # backprop and update the parameters
                    ...
                    new_rows = last_rows[-1, :] + self.avg_loss
                    try:
        ...
        for epoch in range(config.max_epochs):
            ...
            if (self.config.epoch_save_frequency > 0 and epoch % self.config.epoch_save_frequency == 0) or (epoch == config.max_epochs - 1):
                ...
                raw_model = self.model.module if hasattr(
                    self.model, "module") else self.model
                data_dic = raw_model.state_dict()

                raw_model = self.model.module if hasattr(
                    self.model, "module") else self.model
                cfg = raw_model.config
                data_dic["intput.parameter"] = torch.from_numpy(np.array([cfg.ctx_len,cfg.n_layer,cfg.n_embd,
                self.config.batch_size,
                self.config.epoch_save_frequency,
                0.,
                0.]))
                torch.save(data_dic,
                           self.config.epoch_save_path + str(epoch+1) + f"-{self.avg_loss:.6f}" + '.pth')
